...
- **Talk to Claude about the selection.** With an element picked, switch back to chat and say "the selected element should ..." for a single-node ask, or pick a wrapper element (a `div` / `section` / container) and say "the selected area ..." for region-level refactors. Same picker, different framing.
